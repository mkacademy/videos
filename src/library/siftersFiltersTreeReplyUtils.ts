...
import {
  ...
  type ViaContentSiftersFiltersBossesTreeArgs,
  type ViaContentSiftersFiltersMinionsTreeArgs,
  type ViaContentSiftersFiltersUnderbossesTreeArgs,
} from './comments-api-trees';
import type { CommentsTreeReplyBaseArgs, CommentsTreeReplyPayloadResult } from './commentsTreeReplyTypes';

export function getSiftersFiltersTreeReplyPayload({
  ...
  const pathKey = `siftersFilters${roleType}` as keyof typeof COMMENTS_TREE_PATHS.viaFilters;
  const path = COMMENTS_TREE_PATHS.viaFilters[pathKey] as CommentsTreePath;
  ...
  if (roleType === 'Bosses') {
    ...
    const payload: ViaContentSiftersFiltersBossesTreeArgs = {
      siftersFilters: {
        parentIds: parentIDs,
        isPrivateView: false,
        childIds: [commentId],
  ...
  if (roleType === 'Minions') {
    ...
    const payload: ViaContentSiftersFiltersMinionsTreeArgs = {
      siftersFilters: {
        parentIds: parentIDs,
        isPrivateView: false,
        childIds: [commentId],
  ...
  const payload: ViaContentSiftersFiltersUnderbossesTreeArgs = {
    siftersFilters: {
      parentIds: parentIDs,
      isPrivateView: false,
      childIds: [commentId],
